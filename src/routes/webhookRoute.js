...
// MongoDB version of Stripe webhook handler
import stripe from '../config/stripe.js';
import express from 'express';
...
import { allocateTokensToUser } from '../controllers/blockchainController.js';
import { joinLuckyDraw } from '../controllers/luckyDrawController.js';
...
router.post('/webhook', async (req, res) => {
  ...
  if (event.type === 'payment_intent.succeeded') {
    ...
    const entryCount = paymentIntent.metadata?.entryCount;
    ...
    try {
      if (packageId) {
        ...
        const { distance, eventId } = pkg;
        const achievement = await Achievement.findOne({ pkg_id: packageId });
        const totalMilestones = achievement?.milestone_count || 0;

        joinLuckyDraw(eventId, firebaseUID);
        ...
        try {
          const result = await allocateTokensToUser(firebaseUID, 'packages', 'Package reward', { pkgId: packageId });
          console.log(result.message);
        } catch (err) {
          console.error('❌ Failed to allocate tokens:', err);
        }
        ...
        for (let i = 0; i < entryCount; i++) {
          luckyDraw.participants.push(firebaseUID);
        }
